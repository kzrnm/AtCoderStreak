    ...
    public class AtCoderParserTests
    {
        ...
        public void TestParseCookie_Failed()
        {
            ...
            act.ShouldThrow<ArgumentException>()
               .Message.ShouldBe("invalid cookie (Parameter 'cookie')");
        }
        ...
        public void TestParseCookie()
        {
            ...
            parser.ParseCookie(cookie)
                .ShouldBe(("zzzzzzxxxxxxxxxyyyyyyyyy=", "usernam"));
        }

        public static TheoryData<string, string[]> SourceTestFilterREVEL_SESSION => new()
            {
                {
                    null,
                    ["foo=bar", "ban=bon"]
                },
                {
                    ...
                    ["foo=bar", "ban=bon", "REVEL_SESSION=abcdzyxwvu"]
                },
            };
        [Theory]
        [MemberData(nameof(SourceTestFilterREVEL_SESSION))]
        public void TestFilterREVEL_SESSION(string expected, string[] input)
        {
            parser.FilterREVEL_SESSION(input).ShouldBe(expected);
        }

        public static TheoryData<string, string> SourceTestParseLoginCSRFToken => new()
            {
                {
                    null,
                    @"<html><body></body></html>"
        ...
        public async Task TestParseLoginCSRFToken(string expected, string input)
        {
            ...
            var ret = await parser.ParseLoginCSRFToken(ms, TestContext.Current.CancellationToken);
            ret.ShouldBe(expected);
        }


        public static TheoryData<DateTimeOffset?, string> SourceTestParseOldestSubmissionTime => new()
            {
                {
                    null,
                    @"
...
<div class=""panel-submission""><div><span><span class=""fixtime-second"">2018-06-01 19:49:31+0900</span></span></div></div>
        ...
        {
            using var ms = Util.StringToStream(input);
            var ret = await parser.ParseOldestSubmissionTime(ms, TestContext.Current.CancellationToken);
            ret.ShouldBe(expected);
        }

        public static TheoryData<string, string> SourceTestParseLatestSubmissionId => new()
            {
                {
                    null,
                    @"
...
<tr><td class=""submission-score"" data-id=""103566"">100</td></tr>
        ...
        {
            using var ms = Util.StringToStream(input);
            var ret = await parser.ParseFirstSubmissionId(ms, TestContext.Current.CancellationToken);
            ret.ShouldBe(expected);
        }

        public static TheoryData<SerializableSubmissionStatus, string> SourceTestDeserializeSubmissionDetail => new()
            {
                {
                    new SerializableSubmissionStatus{
        ...
        {
            using var ms = Util.StringToStream(input);
            var ret = await parser.DeserializeSubmissionDetail(ms, TestContext.Current.CancellationToken);
            ret.ShouldBe(expected);
        }
        ...
        public async Task TestDeserializeSubmitsJson()
        {
            ...
            var submits = await parser.DeserializeProblemsSubmitAsync(ms, TestContext.Current.CancellationToken);
            submits
                .ShouldBe([
            ...
            submits.Select(s => s.DateTime.Kind)
                .ShouldAllBe(k => k == DateTimeKind.Unspecified);
        }
    }
}
